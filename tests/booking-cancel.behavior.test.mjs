...
async function seed(db, { code, phoneNormalized, status = "new", orgId = 1, dob = "1990-05-05" }) {
  await db.prepare(
    ...
    "2026-09-01", "10:00", status, dob, "civilian", orgId).run();
}
...
test("a patient cannot cancel someone else's booking", async () => {
  await withD1(async (db) => {
    await seed(db, { code: "RD-OWN0000002", phoneNormalized: "380971112233", dob:"1990-05-05" });
    await seed(db, { code: "RD-OTHER00001", phoneNormalized: "380975556677", dob:"1985-04-04" });
    const cookie = await seedPatientSession(db, "380971112233");
