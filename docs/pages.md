# RadiologyOS Pages Map

## Архітектурна межа

```text
BAS Small Company = бізнесове ядро.
RadiologyOS = BAS-подібне ядро + медичні модулі.
Публічний сайт = зовнішня вітрина і канал запису.
```

Публічні маршрути не є фінансовим registrar-рівнем: вони можуть створити intake/booking і технічну `pending` платіжну транзакцію, але проведена оплата/повернення виникає лише у business-core після trusted confirmation.

## 1. Поточна публічна частина

### `/site/` або `index.html`
Головна сторінка.

Блоки (порядок IA згори донизу):
- шапка з госпіталем і входом;
- hero-lead: коротке «що це» + одна головна CTA «Записатися на дослідження»;
- «Кому підходить»: дві картки аудиторій (військовослужбовці → `military.html`, цивільні → `price.html`);
- «Які дослідження проводимо»: перелік модальностей без цін (тарифи лишаються на `price.html`);
- «Яке дослідження що показує»: освітній блок із трьох реальних напрямів (флюорографія, рентгенографія, КТ) з коротким «що показує» й посиланнями на сторінки модальностей; несе дисклеймер «довідка, не медична рекомендація». Лише те, що є в прайсі — без МРТ/ПЕТ/мамографії тощо (див. lessons #21);
- «Як записатися»: розгорнуті сценарії для військовослужбовців і цивільних (без акордеонів; вкладена підготовка до барію лишається `details`);
- «Про відділення»: опис + показники роботи (social proof, після практичних блоків);
- «Часті запитання»: єдиний ексклюзивний accordion (`details name="faq"`);
- контакти.

Онлайн-заявка відкривається в шухляді «Моя заявка» (кошик), а не окремим блоком головної.

### `price.html`
Прайс-лист.

Функції:
- групи послуг;
- описи;
- ціни;
- додавання до кошика;
- перехід до заявки.

### `military.html`
Сторінка для військовослужбовців.

Блоки:
- безоплатні дослідження;
- порядок проходження;
- планові/екстрені дослідження;
- отримання результатів.

### `cabinet.html`
Демонстраційний кабінет пацієнта.

Важливо:
- на Web MVP не є захищеним медичним кабінетом;
- не зберігати реальні дані.

### `/api/pay-link`
Публічний/patient-facing канал ініціації оплати.

- сервер сам визначає booking/суму в tenant + patient identity scope;
- може створити/повернути технічний `payment_transactions.status = pending`;
- **не** створює `business_documents(payment)`;
- **не** створює `cash_movements` або `patient_settlement_movements`;
- господарський факт виникає лише після trusted provider/staff settlement.

## 2. Поточна внутрішня частина

### `staff.html`
Фактично містить кілька модулів в одному файлі:

- заявки;
- черга на сьогодні;
- розклад;
- прайс;
- відділення;
- комунікації та оплата;
- налаштування;
- редагування персоналу;
- редагування обладнання;
- редагування текстів сайту;
- протоколи;
- синхронізація Google Apps Script.

### `/staff/inventory`
Перший внутрішній модуль, переведений на BAS-подібну документну модель.

Вкладки:
- **Залишки** — номенклатура, залишки, партії та створення чернеток складських документів;
- **Документи** — журнал `Надходження матеріалів` / `Списання матеріалів`, картка документа, стани, проведення, скасування та друк;
- **Рухи** — read-only регістр складських рухів із посиланням на документ-реєстратор; старі неінферовані записи позначаються `Legacy`; серверний фільтр періоду Від/До (`date(created_at)`), дата руху відформатована через `fmtDate`.

Новий складський документ створюється як `draft`; залишки змінюються лише після `posted`.

Вкладки — коректний ARIA tablist (roving `tabIndex`, стрілки/Home/End, `role="tabpanel"`); пов'язані розділи (інвентаризація, склади) — окремий `<nav>` зі справжніми `<a href>`. Перехід із руху-переміщення веде на конкретне переміщення (`?id=`).

### `/staff/inventory/print?id=<documentId>`
Захищена друкована форма складського документа.

- формує/повторно використовує tenant-scoped immutable snapshot;
- показує версію шаблону, snapshot ID та скорочений SHA-256;
- підтримує браузерний друк / Save as PDF;
- для проведеного документа повторний друк відтворює канонічний snapshot, а не поточні змінені дані довідників.

### `/staff/finance`
Внутрішній BAS-подібний фінансовий workspace для реєстратора/адміністратора.

Вкладки:
- **Документи** — `Оплата` / `Повернення`, номер, booking, пацієнт, послуга, метод, сума, стан, друк;
- **Рухи грошей** — read-only `cash_movements` з документом-реєстратором;
- **Взаєморозрахунки** — агрегація `patient_settlement_movements` по booking/patient.

Вкладки — коректний ARIA tablist (roving `tabIndex`, стрілки/Home/End, `role="tabpanel"`); пов'язані розділи (каси, надані послуги, усі документи, обороти регістрів) — окремий `<nav>` зі справжніми `<a href>`, не вкладки.

Підсумки сторінки («Надійшло / Повернено / Чистий рух») рахуються серверно по **всьому** cash register (`summarizeCashRegister`), розбиті за валютами (додаткові валюти — окремим рядком), а не з завантаженого вікна. Фільтр періоду (Від/До на `occurred_at`) скоупить і три вкладки, і підсумки; є клієнтська перевірка `Від ≤ До` й кнопка «Скинути період». Історичні paid/refunded `payment_transactions` без document links показуються окремим Legacy count і не домішуються в нові регістрові підсумки.

### `/staff/finance/print?id=<documentId>`
Захищена A4 квитанція `payment_receipt` для `payment` або `refund`.

- tenant/RBAC guarded;
- template v1;
- immutable `printed_form_snapshots` + SHA-256;
- snapshot містить document/booking/patient/service/payment metadata;
- refund snapshot містить первинний payment document як підставу, якщо він існує;
- повторний друк проведеного документа відтворює перший canonical snapshot конкретного state/version.

### `/staff/shifts`
Tenant-scoped графік змін персоналу — сучасний веб-порт логіки Calendar6.

- 8 пресетів із наданого `Setting.mdb`;
- циклічні матриці бригад з явною опорною датою;
- місячна таблиця `працівник × день`;
- денні/вечірні/нічні/добові зміни, вихідні та персональні корекції;
- відпустка/лікарняний/інша ручна позначка;
- CSV та браузерний друк;
- `department_head`/legacy `admin` керують графіками tenant; інші operational staff читають тільки власний графік;
- `organization_admin` не отримує кадрово-операційне керування через control-plane роль.

Детальна модель і provenance Calendar6: `docs/staff-shift-calendar.md`.

### `/staff/directories/personnel-refs`
Кадрові довідники посад (`personnel_positions`) і звань (`personnel_ranks`),
tenant-scoped.

- керування (додати / перейменувати / приховати) — `admin` або `department_head`;
- значення живлять datalist-підказки у формі працівника (`/staff/personnel`);
- поля картки лишаються free-text, тож приховання значення не змінює історію;
- редагується без деплою (раніше списки були захардкоджені в клієнті).

### `/staff/personnel/print?id=<personnelId>`
Друкована кадрова картка — версійований immutable snapshot (`personnel_card_snapshots`).

- POST будує самодостатній payload (особа + призначення + графік), рахує SHA-256
  і зберігає незмінний snapshot; однаковий рендер згортається в один рядок;
- відтворюється з конкретного snapshot навіть після зміни картки;
- A4-друк браузером (Save as PDF), без окремого R2/PDF-контуру;
- права — `admin` / `department_head`; кнопка «Друк картки» в редакторі працівника.

### `/staff/sheet-preview`
Прототип оформлення внутрішніх журналів у стилі Google Sheets / Excel (макет для узгодження вигляду).

- **ізольований** — жодного API, дані умовні (не реальні пацієнти), стилі інлайнові й не чіпають глобальний CSS;
- демонструє: літери стовпців, заморожену шапку та номерний гутер, лінії комірок, зебру, виділення рядка, чипи статусу/оплати, моно-цифри з вирівнюванням праворуч, статус-бар із сумою, світлу й темну теми;
- призначення — затвердити табличний скін перед розкочуванням на спільний `DataTable`/`JournalForm`; це **не робоча сторінка**;
- `noindex,nofollow` успадковано з `app/staff/layout.tsx`.

## 3. Цільова внутрішня маршрутизація

```text
/staff
├── dashboard
├── applications
├── schedule
├── shifts
├── queue
├── patients
├── studies
├── protocols
├── finance
├── reports
├── staff
├── rooms
├── equipment
├── services
├── inventory
├── tasks
├── exports
└── settings
```

## 4. Доступ за ролями

- Dashboard: завідувач, старший рентгенлаборант, адміністратор.
- Applications: реєстратор, адміністратор.
- Schedule: реєстратор, старший рентгенлаборант, лікарі, лаборанти.
- Staff shifts: керування — `department_head` або legacy `admin`; власний графік read-only — авторизований operational/self-service staff; system `organization_admin` не керує змінами.
- Patients: реєстратор, лікарі, дозволений персонал.
- Protocols: лікар-рентгенолог, лікар УЗД.
- Equipment: старший рентгенлаборант, інженер, адміністратор.
- Inventory: читання — авторизований staff; створення/проведення складських документів — адміністратор або рентгенолаборант згідно з runtime RBAC поточного модуля.
- Finance: фінансовий журнал, ручне підтвердження оплати, повернення й друк фінансових форм — ролі, для яких `canManageFinance` повертає true (поточний staff contract: реєстратор/адміністратор).
- Reports: завідувач, адміністратор.
- Settings: системний адміністратор.

## 5. SEO

- Публічні сторінки індексуються.
- `staff`, `cabinet`, службові сторінки — `noindex,nofollow`.
