...
export default function EquipmentRegistryPage(){
  ...
  useEffect(()=>{let active=true;const t=window.setTimeout(async()=>{const res=await fetch("/api/staff/equipment-registry",{cache:"no-store"});const data=await res.json().catch(()=>({})) as {equipment?:EquipmentRecord[];staff?:StaffInfo;error?:string};if(!active)return;if(!res.ok){setError(data.error||"Не вдалося завантажити обладнання");setLoaded(true);return;}if(data.equipment)setEquipment(data.equipment);if(data.staff)setStaff(data.staff);setLoaded(true);},0);return()=>{active=false;window.clearTimeout(t)}},[]);
  ...
  return <StaffWorkspaceShell active="equipment" title="Обладнання" description="Окремий реєстр апаратів, які зараз використовує відділення." staffName={staff?.displayName} staffRole={staff?.role}>
    {!loaded?<p className="notice">Завантаження…</p>:error&&!staff?<p className="notice error">{error}</p>:<form className="equipmentRegistry" onSubmit={save}>
      <div className="equipmentRegistryHead"><div><b>Реєстр обладнання</b><span>{equipment.filter(row=>row.status==="active").length} апаратів у роботі · характеристики можна доповнювати</span></div><a href="/staff/schedule">Перейти до графіка кабінетів →</a></div>
      <div className="equipmentRegistryGrid">{equipment.map((row,index)=><details className="equipmentCard" key={row.id} open={index===0}>
      ...
      {staff?.role==="admin"?<button className="equipmentSave" type="submit">Зберегти обладнання</button>:<p className="settingsHint">Перегляд доступний персоналу; редагування — адміністратору.</p>}
    </form>}
  </StaffWorkspaceShell>;
}
