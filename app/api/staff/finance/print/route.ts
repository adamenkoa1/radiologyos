...
async function renderPayload(db:D1Database,organizationId:number,documentId:number) {
  const finance=await getFinanceDocument(db,organizationId,documentId);
  if(!finance) return null;
  const [organization,booking,source]=await Promise.all([
    db.prepare("SELECT name FROM organizations WHERE id=? LIMIT 1")
      .bind(organizationId).first<{name:string}>(),
    db.prepare(
      `SELECT code,name,service,service_code AS serviceCode,patient_category AS patientCategory
       FROM bookings WHERE organization_id=? AND id=? LIMIT 1`
    ).bind(organizationId,finance.detail.bookingId).first<{
      code:string;name:string;service:string;serviceCode:string;patientCategory:string;
    }>(),
    finance.detail.sourceDocumentId
      ? db.prepare(
          `SELECT number,document_type AS documentType FROM business_documents
           WHERE organization_id=? AND id=? LIMIT 1`
        ).bind(organizationId,finance.detail.sourceDocumentId).first<{number:string;documentType:string}>()
  ...
  return {
    ...
    document:{
      ...
      number:finance.document.number,
      documentType:finance.document.documentType,
      occurredAt:finance.document.occurredAt,
      state:finance.document.state,
      comment:finance.document.comment,
      createdBy:finance.document.createdBy,
      createdAt:finance.document.createdAt,
      postedBy:finance.document.postedBy,
      postedAt:finance.document.postedAt,
    },
    booking:{
      id:finance.detail.bookingId,
      code:booking.code,
      patientName:booking.name,
      patientId:finance.detail.patientId,
      service:booking.service,
      serviceCode:booking.serviceCode,
      patientCategory:booking.patientCategory,
    },
    payment:{
      amount:finance.detail.amount,
      currency:finance.detail.currency,
      method:finance.detail.method,
      provider:finance.detail.provider,
      providerReference:finance.detail.providerReference,
      transactionStatus:finance.transaction?.status || "",
    },
...
async function financeContext(request:Request) {
  ...
  if(!canManageFinance(ctx.member.role)) {
    return {response:Response.json({error:"Друк фінансових документів доступний реєстратору або адміністратору"},{status:403})} as const;
  }
...
export async function GET(request:Request) {
  ...
  const row=await db.prepare(
    `SELECT s.id,s.document_id AS documentId,s.form_type AS formType,s.template_version AS templateVersion,
     ...
     WHERE s.organization_id=? AND s.id=? AND s.form_type=?
       AND d.document_type IN ('payment','refund') LIMIT 1`
...
export async function POST(request:Request) {
  ...
  const finance=await getFinanceDocument(db,ctx.organizationId,documentId);
  if(!finance) return Response.json({error:"Фінансовий документ не знайдено"},{status:404});
  ...
  if(!payload) return Response.json({error:"Фінансовий документ не знайдено"},{status:404});
